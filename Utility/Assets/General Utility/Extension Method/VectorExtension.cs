...
namespace Astek
{
    public static class VectorExtension
    {
        ...
        public static Vector3 SpdLerp(Vector3 source, Vector3 destination, float startTime, float speed, EasingFunction easingFunction = EasingFunction.Linear)
        {
            // 1. Calculate distance covered based on current speed
            float distanceCovered = (Time.time - startTime) * speed;
            // 2. Calculate fraction of journey (t)
            float fractionOfJourney = distanceCovered / Math.Max(0.000000000001f, source.SqrMagnitudeDistance(destination));
            return Vector3.Lerp(source, destination, fractionOfJourney);
        }
