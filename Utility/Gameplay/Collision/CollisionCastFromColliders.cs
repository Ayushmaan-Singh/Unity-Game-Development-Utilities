...
namespace Astek.Gameplay.Collision
{
    public static class CollisionCast2D
    {
        ...
        public static RaycastHit2D Cast(Collider2D collider2D, Vector2 direction, LayerMask collisionMask) =>
            Cast(collider2D, direction, direction.magnitude, collisionMask);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RaycastHit2D Cast(Collider2D collider2D, Vector2 direction, float distance, LayerMask collisionMask)
        {
        ...
        public static RaycastHit2D[] CastAll(Collider2D collider2D, Vector2 direction, LayerMask collisionMask) =>
            CastAll(collider2D, direction, direction.magnitude, collisionMask);
    ...
    public static class CollisionCast
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RaycastHit Cast(Collider collider, Vector3 direction, float distance, LayerMask collisionMask)
        {
            if (!collider)
            ...
            return hit.collider != collider ? hit : default;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RaycastHit Cast(Collider collider, Vector3 direction, LayerMask collisionMask) =>
            Cast(collider, direction, direction.magnitude, collisionMask);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RaycastHit[] CastAll(Collider collider, Vector3 direction, float distance, LayerMask collisionMask)
        {
            if (collider == null)
                throw new ArgumentNullException(nameof(collider));
            ...
            return hits.Where(x => x.collider != collider).ToArray();
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RaycastHit[] CastAll(Collider collider, Vector3 direction, LayerMask collisionMask) =>
            CastAll(collider, direction, direction.magnitude, collisionMask);
